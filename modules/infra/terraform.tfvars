# === Configuration Proxmox ===
proxmox_host = "192.168.1.100"
proxmox_node = "devsecops-dojo"

# === IDs et réseau ===
vm_id_start     = 110
network_bridge  = "vmbr0"
ip_address_base = "192.168.1"
ip_start        = 110
gateway         = "192.168.1.1"
nameserver      = "192.168.1.1"

# === Ressources Rancher ===
rancher_cpu_cores = 2
rancher_memory    = 8192  # 8 GB RAM
rancher_disk_size = 60    # 60 GB

# === Clé SSH ===
ssh_public_key = "ssh-rsa  nsfab@gaby"

# ========================================
# Résultat attendu:
# rancher-1 → ID 110 → 192.168.1.110 → 2C/8GB/60GB
# rancher-2 → ID 111 → 192.168.1.111 → 2C/8GB/60GB
# rancher-3 → ID 112 → 192.168.1.112 → 2C/8GB/60GB
